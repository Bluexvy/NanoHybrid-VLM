    ...
    int gdn_index)
{
    ...
    if (kernel_kind == KernelKind::WarpStateStaging) {
        size_t shared_state_bytes =
            static_cast<size_t>(arguments.key_dim)
            * arguments.value_dim
            * sizeof(float);
    ...
    } else if (kernel_kind == KernelKind::HybridReduction) {
...
float measure_kernel(KernelKind kernel_kind, const KernelArguments& arguments, int batch_size, size_t active_state_bytes)
{
    ...
    for (int iteration = 0; iteration < warmup_iterations; ++iteration) {
        for (int gdn_index = 0; gdn_index < arguments.num_layers; ++gdn_index) {
            launch_kernel(
                kernel_kind,
                arguments,
                batch_size,
    ...
    KernelKind compared_kernel)
{
    ...
    std::vector<float> warp_output(output_elements);
    std::vector<float> staging_output(output_elements);

    check_cuda(cudaMemset(arguments.state_pool, 0, active_state_bytes));
    for (int gdn_index = 0; gdn_index < arguments.num_layers; ++gdn_index) {
        launch_kernel(KernelKind::FullWarpShuffle, arguments, batch_size, gdn_index);
    }
    check_cuda(cudaGetLastError());
    check_cuda(cudaMemcpy(warp_output.data(), arguments.output, output_bytes, cudaMemcpyDeviceToHost));

    check_cuda(cudaMemset(arguments.state_pool, 0, active_state_bytes));
    for (int gdn_index = 0; gdn_index < arguments.num_layers; ++gdn_index) {
        launch_kernel(compared_kernel, arguments, batch_size, gdn_index);
    }
    check_cuda(cudaGetLastError());
    check_cuda(cudaMemcpy(staging_output.data(), arguments.output, output_bytes, cudaMemcpyDeviceToHost));

    float max_error = 0.0f;
    for (size_t index = 0; index < output_elements; ++index) {
        max_error = std::max(max_error, std::fabs(warp_output[index] - staging_output[index]));
    }
    return max_error;
}
...
int main()
{
    ...
    std::printf(
        "State Staging dynamic shared memory: %.2f KiB\n",
        static_cast<double>(shared_state_bytes) / 1024.0
    );

    std::printf("GPU: %s\n", properties.name);
    std::printf("Shape: H=%d, Dk=%d, Dv=%d\n", num_heads, key_dim, value_dim);
    std::printf("State layout: [B, %d, H, Dk, Dv], FP32\n", num_layers);
    std::printf("Maximum 24-layer state pool: %.2f GiB\n", static_cast<double>(state_bytes) / 1024.0 / 1024.0 / 1024.0);
    std::printf("Each result is the median of %d measurements.\n\n", repeats);
    std::printf("     B | Warp step us | Staging step us | Warp layer us | Staging layer us |    change | speedup | max error\n");
    std::printf("-------+--------------+-----------------+---------------+------------------+-----------+---------+----------\n");
    for (int batch_size : batch_sizes) {
        ...
        std::vector<float> staging_times;

        for (int repeat = 0; repeat < repeats; ++repeat) {
            if (repeat % 2 == 0) {
                warp_times.push_back(measure_kernel(
                ...
                staging_times.push_back(measure_kernel(
                    KernelKind::WarpStateStaging,
                    arguments,
                    batch_size,
                    active_state_bytes
                ));
        ...
        float staging_us = median(staging_times);

        double staging_change =
            (static_cast<double>(staging_us) / warp_us - 1.0)
            * 100.0;

        double speedup =
            static_cast<double>(warp_us) / staging_us;

        float warp_layer_us = warp_us / static_cast<float>(num_layers);
        float staging_layer_us = staging_us / static_cast<float>(num_layers);

        float max_error = compare_one_step_outputs(
            arguments,
            batch_size,
            active_state_bytes,
            KernelKind::WarpStateStaging
        ...
        std::printf(
            "%6d | %12.3f | %15.3f | %13.3f | %16.3f | %+8.2f%% | %7.3fx | %.2e\n",
            batch_size,
            warp_us,
            staging_us,
            warp_layer_us,
