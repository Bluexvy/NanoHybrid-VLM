    ...
    int gdn_index)
{
    ...
    float query_inverse_norm = 1.0f / sqrtf(shared_query_squared[0] + 1e-6f);
    float key_inverse_norm = 1.0f / sqrtf(shared_key_squared[0] + 1e-6f);
    float query_scale = 1.0f / sqrtf(static_cast<float>(key_dim));

    if (thread_index < key_dim) {
        shared_query[thread_index] = shared_query[thread_index] * query_inverse_norm * query_scale;
        shared_key[thread_index] = shared_key[thread_index] * key_inverse_norm;
    }
...
int main()
    ...
    const char* kernel_name = "hybrid_reduction";
#else
    ...
    for (int case_index = 0; case_index < num_batch_cases; ++case_index) {
        ...
        */
        ...
        std::printf(
            "B=%2d | blocks=%4d | avg=%9.3f us | recurrent tok/s=%10.2f | checksum=% .6e\n",
            batch_size,
            blocks,
            average_us,
